...
export class Input {
    constructor(name, label, type = 'text', col = 4, require = false, value = '', disable = false, id = null, mascName = null) {
    ...
    render() {
        ...
        <div class=" col-span-4  lg:col-span-${this.col}">
            ...
            class="${this.type != 'color' ? inputCss : colorCss}" 
            name="${this.name}" 
            ${this.require && 'required'} 
            oninput="this.value=globalThis?.['${this.mascName}'] ? globalThis?.['${this.mascName}']?.(this.value) : this.value;globalThis.Dados[this.name]=this.value" >
        </div>    
        `
    }
...
export class Input2 {
    ...
    render() {
        ...
        <input 
            type="${this.type}" 
            value="${globalThis.Dados[this.name] || ''}" 
            class="flex-shrink flex-grow flex-auto leading-normal w-px flex-1 border h-10 border-blue rounded rounded-r-none px-3 relative" 
            name="${this.name}" 
            ${this.require && 'required'} 
            oninput="globalThis.Dados[this.name]=this.value">
